...
func TestCheckVersion(t *testing.T) {
	...
	t.Run("skip_if_no_launcher_version_or_latest_version", func(t *testing.T) {
		...
		t.Setenv(envir.FleekLatestVersion, "")
		buf := new(bytes.Buffer)
		CheckVersion(buf, "fleek info")
		if buf.String() != "" {
	...
	t.Run("print_if_binary_version_outdated", func(t *testing.T) {
		...
		t.Setenv(envir.FleekLatestVersion, "0.4.9")

		// mock the existing binary version
		currentFleekVersion = "v0.4.8"

		buf := new(bytes.Buffer)
		CheckVersion(buf, "fleek info")
		if !strings.Contains(buf.String(), "New fleek available") {
			t.Errorf("expected notice about new fleek version, got %q", buf.String())
		}
	})
	...
	t.Run("skip_if_all_versions_up_to_date", func(t *testing.T) {
		...
		t.Setenv(envir.FleekLatestVersion, "0.4.8")

...
// We set this env-var so that this fleek command invoking other fleek commands
// do not re-run the version check and print the notice again.
const envName = "__FLEEK_VERSION_CHECK"

// currentFleekVersion is the version of the fleek CLI binary that is currently running.
// We use this variable so that we can mock it in tests.
var currentFleekVersion = build.Version
...
func CheckVersion(w io.Writer, commandPath string) {
	...
	if launcherNotice != "" {
		...
		// fallthrough to alert the user about a new Fleek CLI binary being possibly available
	}

	// check fleek CLI version
	fleekNotice := fleekVersionNotice()
	if fleekNotice != "" {
		ux.Finfo(w, fleekNotice)
	}

	os.Setenv(envName, "1")
}

// SelfUpdate updates the fleek launcher and fleek CLI binary.
...
// for fleek. The production fleek application is actually this launcher script
...
func SelfUpdate(stdOut, stdErr io.Writer) error {
	...
	return selfUpdateFleek(stdErr)
}

func selfUpdateLauncher(stdOut, stdErr io.Writer) error {
	installScript := ""
	if cmdutil.Exists("curl") {
		installScript = "curl -fsSL https://getfleek.dev/fleek | bash"
	} else if cmdutil.Exists("wget") {
		installScript = "wget -qO- https://getfleek.dev/fleek | bash"
	} else {
		return usererr.New("curl or wget is required to update fleek. Please install either and try again.")
	}

	// Delete current version file. This will trigger an update when invoking any fleek command;
	// in this case, inside triggerUpdate function.
	if err := removeCurrentVersionFile(); err != nil {
		return err
	}

	// Fetch the new launcher. And installs the new fleek CLI binary.
	...
	printSuccessMessage(stdErr, "Fleek", currentFleekVersion, updated.fleekVersion)

	return nil
}

// selfUpdateFleek will update the fleek CLI binary to the latest version.
func selfUpdateFleek(stdErr io.Writer) error {
	// Delete current version file. This will trigger an update when the next fleek command is run;
	// in this case, inside triggerUpdate function.
	if err := removeCurrentVersionFile(); err != nil {
		return err
	}
...
// triggerUpdate runs `fleek version -v` and triggers an update since a new
...
func triggerUpdate(stdErr io.Writer) (*updatedVersions, error) {
	...
	if exePath == "" {
		ux.Fwarning(stdErr, "expected FLEEK_LAUNCHER_PATH to be set. Defaulting to \"fleek\".")
		exePath = "fleek"
	}

	// TODO savil. Add a --json flag to fleek version and parse the output as JSON
	...
	// Parse the output to ascertain the new fleek and launcher versions
	updated := &updatedVersions{}
	for _, line := range strings.Split(buf.String(), "\n") {
		if strings.HasPrefix(line, "Version:") {
			updated.fleekVersion = strings.TrimSpace(strings.TrimPrefix(line, "Version:"))
		}
...
func launcherVersionNotice() string {
	...
	return fmt.Sprintf(
		"New launcher available: %s -> %s. Please run `fleek version update`.\n",
		currentLauncherVersion(),
		expectedLauncherVersion,
	)
}

func fleekVersionNotice() string {
	if !isNewFleekAvailable() {
		return ""
	}

	return fmt.Sprintf(
		"New fleek available: %s -> %s. Please run `fleek version update`.\n",
		currentFleekVersion,
		latestVersion(),
	)
}
...
func isNewFleekAvailable() bool {
	latest := latestVersion()
	if latest == "" {
		return false
	}
	return semverCompare(currentFleekVersion, latest) < 0
}
...
func removeCurrentVersionFile() error {
	...
	currentVersionFilePath := filepath.Join(xdg.CacheSubpath("fleek"), "current-version")
...
func latestVersion() string {
	version := os.Getenv(envir.FleekLatestVersion)
	if version == "" {
		return ""
	}
	return "v" + version
